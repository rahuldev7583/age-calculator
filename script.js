...
document.getElementById("head").innerHTML =
  "Hey " + name[0].toUpperCase() + name.slice(1).toLowerCase();
let date = new Date();
...
function run() {
  ...
  function diff() {
    if (cDay >= bDay && cMonth >= bMonth) {
      result = `<p class="bpara">You are ${cYear - bYear} years ${
        cMonth - bMonth
      } months and
    ${cDay - bDay} days old.</p>`;
    //  console.log(bYear, bMonth, bDay + " current " + cYear, cMonth, cDay);
   ...
   //   console.log(bYear, bMonth, bDay + " current " + cYear, cMonth, cDay);
      ...
      rMonth = cMonth + 12 - bMonth;
      rYear = cYear - 1 - bYear;
      result = `<p class="bpara">You are ${rYear} years ${rMonth} months and
      ${rDay} days old.</p>`;
  ...
  //    console.log(bYear, bMonth, bDay + " current " + cYear, cMonth, cDay);
    ...
    let sen = `<p class="para">If you sleep 8hr per day which we most of us do. <span>It means you slept ${sleepYear} years ${sleepMonth} months and ${sleepDay} days.</span> We spend around 2hr per day on eating, bathing, cleaning etc and nowadays we spend around 2hr per day on our phone. It means you spend ${
      sleepYear / 2
    } years ${sleepMonth / 2} months and ${
      sleepDay / 2
    } days on just eating, bathing and scrolling your phone.</p>`;
    return result + sen;
  }
